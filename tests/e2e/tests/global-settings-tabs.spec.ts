...
test.describe('Global Settings Tabbed Layout', () => {
  ...
  test('OpenTherm tab should show gateway dropdown or a no-gateways alert', async ({ page }) => {
    // Open OpenTherm tab (label may be 'OpenTherm' per translations)
    const openthermTab = page.getByRole('tab', { name: /OpenTherm/i });
    await openthermTab.click();
    await page.waitForTimeout(500);

    // Check for dropdown or alert
    const select = page.locator('div[aria-labelledby="opentherm-gateway-select-label"]');
    const alert = page.getByText(/No OpenTherm gateways found|No OpenTherm gateways found/i);
  ...
  test('should allow selecting an OpenTherm Gateway and saving', async ({ page }) => {
    const openthermTab = page.getByRole('tab', { name: /OpenTherm/i });
    await openthermTab.click();
    await page.waitForTimeout(500);

    const select = page.getByLabel('Gateway Integration ID (slug)');
    const selectVisible = await select.isVisible().catch(() => false);
    if (!selectVisible) {
      // Skip if no gateways are present
      test.skip();
      return;
    }

    // Open the dropdown and select the first option (not 'None')
    await select.click();
    // Options are rendered as role='option'; pick second option (first is None)
