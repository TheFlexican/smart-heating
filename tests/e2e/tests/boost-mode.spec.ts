...
test.describe('Boost Mode Tests', () => {
  ...
  test('should activate boost from zone card quick button', async ({ page }) => {
    // Navigate to home/dashboard
    await page.goto('/');
    await page.waitForTimeout(2000);

    // Find Living Room zone card and look for the rocket icon button
    const livingRoomCard = page.locator('.MuiCard-root:has-text("Living Room")').first();
    await expect(livingRoomCard).toBeVisible();

    // Find and click the rocket icon (boost button) - it's an IconButton with RocketLaunchIcon
    const boostButton = livingRoomCard.locator('button').filter({ has: page.locator('svg[data-testid="RocketLaunchIcon"]') });
    await boostButton.click();
    await page.waitForTimeout(3000);

    // Verify BOOST chip appears on the card
    const boostChip = livingRoomCard.locator('.MuiChip-label:has-text("BOOST")');
    await expect(boostChip).toBeVisible({ timeout: 10000 });

    // Verify boost button changes appearance (should have error.dark background when active)
    const activeBoostButton = livingRoomCard.locator('button').filter({ has: page.locator('svg[data-testid="RocketLaunchIcon"]') });
    await expect(activeBoostButton).toHaveCSS('background-color', /rgb/); // Should have background color
  });

  test('should cancel boost from zone card quick button', async ({ page }) => {
    // Navigate to home/dashboard
    await page.goto('/');
    await page.waitForTimeout(2000);

    // Find Living Room zone card
    const livingRoomCard = page.locator('.MuiCard-root:has-text("Living Room")').first();
    await expect(livingRoomCard).toBeVisible();

    // Ensure boost is active first (from previous test or activate it)
    const boostChip = livingRoomCard.locator('.MuiChip-label:has-text("BOOST")');
    const isBoostActive = await boostChip.isVisible().catch(() => false);
    ...
    // Now click boost button again to cancel
    const boostButton = livingRoomCard.locator('button').filter({ has: page.locator('svg[data-testid="RocketLaunchIcon"]') });
    await boostButton.click();
    await page.waitForTimeout(2000);

    // Verify BOOST chip disappears
    const boostChipAfter = livingRoomCard.locator('.MuiChip-label:has-text("BOOST")');
    await expect(boostChipAfter).not.toBeVisible();
  });

  test('boost button should show tooltip on hover', async ({ page }) => {
    // Navigate to home/dashboard
    await page.goto('/');
    await page.waitForTimeout(2000);
    ...
    // Hover over boost button
    const boostButton = livingRoomCard.locator('button').filter({ has: page.locator('svg[data-testid="RocketLaunchIcon"]') });
    await boostButton.hover();
    await page.waitForTimeout(500);

    // Check for tooltip (either "Click to activate" or "Click to cancel" depending on state)
