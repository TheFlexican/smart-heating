...
async def test_collect_metrics_full_flow(monkeypatch):
    ...
    area_manager.get_all_areas.return_value = {
        "a1": MagicMock(
            current_temperature=20.0, target_temperature=21.0, state="heating"
        ),
        "a2": MagicMock(
            current_temperature=19.0, target_temperature=21.0, state="idle"
        ),
    }
    ...
    collector._async_get_opentherm_metrics = AsyncMock(
        return_value={"outdoor_temp": 5.0}
    )
