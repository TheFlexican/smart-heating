...
class TestInitialization:
    ...
    async def test_async_setup_with_sensors(
    ...
    async def test_async_setup_disabled_sensors(
    ...
    async def test_async_setup_replaces_existing_listener(
        self, safety_monitor: SafetyMonitor, mock_area_manager, hass: HomeAssistant
    ):
    ...
    async def test_async_setup_sensor_does_not_exist(
        self, safety_monitor: SafetyMonitor, mock_area_manager, hass: HomeAssistant
    ):
...
class TestStateChangeHandling:
    ...
    async def test_handle_safety_sensor_state_change(
    ...
    async def test_handle_safety_sensor_state_change_no_new_state(
        self, safety_monitor: SafetyMonitor
    ):
        """Test handling state change with no new state."""
        event = Event(
            "state_changed",
            {"entity_id": "binary_sensor.smoke", "old_state": None, "new_state": None},
        )
...
class TestSafetyChecks:
    ...
    async def test_check_safety_status_no_alert(
        self, safety_monitor: SafetyMonitor, mock_area_manager
    ):
    ...
    async def test_check_safety_status_alert_detected(
        ...
        with patch.object(
            safety_monitor, "_trigger_emergency_shutdown", new_callable=AsyncMock
        ) as mock_shutdown:
            await safety_monitor._check_safety_status()
    ...
    async def test_check_safety_status_alert_cleared(
        self, safety_monitor: SafetyMonitor, mock_area_manager
    ):
...
class TestReconfigure:
    ...
    async def test_async_reconfigure(
        self, safety_monitor: SafetyMonitor, mock_area_manager, hass: HomeAssistant
    ):
