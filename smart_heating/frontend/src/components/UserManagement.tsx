...
export const UserManagement: React.FC<UserManagementProps> = ({ embedded = false }) => {
    ...
    switch (field) {
      ...
      case 'person_entity':
        if (!editingUser && !value)
          return t('users.errors.personEntityRequired', 'Person entity is required')
        return ''
      case 'priority': {
  ...
  const handleDeleteUser = async (userId: string) => {
    if (
      !globalThis.confirm(t('users.confirmDelete', 'Are you sure you want to delete this user?'))
    ) {
      return
    }
    try {
  ...
  const getUserStatus = (userId: string): 'home' | 'away' => {
    ...
    const userKey = Object.keys(userData.users).find(key => userData.users[key].user_id === userId)
      ...
      <Box sx={{ px: embedded ? 0 : 3 }}>
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {userData &&
                Object.entries(userData.users).map(([userId, user]) => (
                  <TableRow key={userId}>
                    <TableCell>
                    ...
                    <TableCell>
                      <Stack direction="row" spacing={0.5} flexWrap="wrap">
                        {Object.entries(user.preset_preferences).map(([preset, temp]) => (
                          <Chip key={preset} size="small" label={`${preset}: ${temp}°C`} />
                        ))}
                      </Stack>
                    </TableCell>
                    <TableCell>
        ...
        <Dialog open={editDialogOpen} onClose={handleCloseDialog} maxWidth="md" fullWidth>
          <DialogTitle>
            {editingUser ? t('users.editUser', 'Edit User') : t('users.createUser', 'Create User')}
          </DialogTitle>
          <DialogContent>
              ...
              {['home', 'away', 'sleep', 'eco', 'comfort', 'activity'].map(preset => (
                <Grid size={{ xs: 6, md: 4 }} key={preset}>
                  <TextField
                    ...
                    onChange={e =>
                      handlePresetTempChange(preset, Number.parseFloat(e.target.value))
                    }
                    slotProps={{
                      htmlInput: { step: 0.5 },
          ...
          <DialogActions>
            <Button data-testid="user-cancel" onClick={handleCloseDialog}>
