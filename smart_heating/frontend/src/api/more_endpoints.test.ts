...
describe('API additional endpoints', () => {
  ...
  it('setHeatingType with optional customOverheadTemp', async () => {
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/smart_heating/areas/a1/heating_type', {
  ...
  it('safety sensor CRUD', async () => {
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/smart_heating/safety_sensor', {
      sensor_id: 's1',
      enabled: true,
    })
  ...
  it('user CRUD and presence', async () => {
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/smart_heating/users', {
      user_id: 'u1',
      name: 'U',
    })
