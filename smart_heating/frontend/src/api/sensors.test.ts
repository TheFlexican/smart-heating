...
describe('API - Sensors', () => {
  ...
  it('add/remove window sensor, presence sensor and global presence', async () => {
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/smart_heating/areas/a1/window_sensors', { entity_id: 'sensor.w1' })
    mockedAxios.delete = vi.fn().mockResolvedValue({ data: {} }) as any
    await s.removeWindowSensor('a1', 'sensor.w1')
    expect(mockedAxios.delete).toHaveBeenCalledWith('/api/smart_heating/areas/a1/window_sensors/sensor.w1')

    mockedAxios.post = vi.fn().mockResolvedValue({ data: {} }) as any
    await s.addPresenceSensor('a1', { entity_id: 'sensor.p1' } as any)
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/smart_heating/areas/a1/presence_sensors', { entity_id: 'sensor.p1' })
    mockedAxios.delete = vi.fn().mockResolvedValue({ data: {} }) as any
    await s.removePresenceSensor('a1', 'sensor.p1')
    expect(mockedAxios.delete).toHaveBeenCalledWith('/api/smart_heating/areas/a1/presence_sensors/sensor.p1')
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/smart_heating/global_presence', { sensors: [{ entity_id: 'sensor.p1' }] })
  })
})
