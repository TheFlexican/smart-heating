...
import { Box, Paper, Typography, Button, IconButton, Chip, TextField, FormControl, Select, MenuItem, Alert } from '@mui/material'
  ...
  return (
    <Box sx={{ mt: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button data-testid="trv-add-button-overview" variant="outlined" size="small" onClick={onOpenAdd}>
      ...
      {area.trv_entities && area.trv_entities.length > 0 ? (
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 2, mt: 1 }}>
          {area.trv_entities.map((trv: any) => {
            const runtime = (trvs || []).find(t => t.entity_id === trv.entity_id) as TrvRuntimeState | undefined
            ...
            return (
              <Paper key={trv.entity_id} sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box sx={{ flex: 1 }}>
                  {!isEditing ? (
                    <>
                      <Typography variant="body1">{name}</Typography>
                      <Typography variant="caption" color="text.secondary">{trv.role || 'both'}</Typography>
                      <Box sx={{ mt: 1 }}>
                        {open !== undefined && (
                          <Chip label={open ? t('areaDetail.trvOpen') : t('areaDetail.trvClosed')} color={open ? 'success' : 'default'} size="small" data-testid={`trv-open-${trv.entity_id}`} />
                        )}
                        <Typography variant="caption" sx={{ ml: 1 }} data-testid={`trv-position-${trv.entity_id}`}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                      <TextField size="small" value={editingTrvName ?? ''} onChange={e => setEditingTrvName(e.target.value)} data-testid={`trv-edit-name-${trv.entity_id}`} />
                      <FormControl size="small">
                        <Select value={editingTrvRole ?? trv.role ?? 'both'} onChange={e => setEditingTrvRole(e.target.value as any)} data-testid={`trv-edit-role-${trv.entity_id}`}>
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                  {!isEditing ? (
                    <>
                      <IconButton aria-label={`edit-${trv.entity_id}`} data-testid={`trv-edit-${trv.entity_id}`} size="small" onClick={() => startEditingTrv(trv)}>
                        <EditIcon />
                      </IconButton>
                      <IconButton aria-label={`delete-${trv.entity_id}`} data-testid={`trv-delete-${trv.entity_id}`} size="small" onClick={() => handleDeleteTrv(trv.entity_id)}>
                        <RemoveCircleOutlineIcon />
                      </IconButton>
                    </>
                  ) : (
                    <>
                      <Button size="small" variant="contained" data-testid={`trv-save-${trv.entity_id}`} onClick={() => handleSaveTrv(trv)}>Save</Button>
                      <Button size="small" data-testid={`trv-cancel-edit-${trv.entity_id}`} onClick={() => cancelEditingTrv()}>Cancel</Button>
                    </>
                  )}
                </Box>
              </Paper>
